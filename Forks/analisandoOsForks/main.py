	...
	for caminho in caminhos:
		...
		for linha in arquivo:
			...
			if ( int(metadado['ahead_by']) > 0):
				open('urlDosForksComCommitsAFrente.txt', 'a').write(str (contadorDeProjetosSpringComCommitsAFrenteDaMaster) +'/'+ metadado['repository'].split('/')[1]+'\n')
				contadorDeProjetosSpringComCommitsAFrenteDaMaster = contadorDeProjetosSpringComCommitsAFrenteDaMaster + 1
	print (contadorDeProjetosSpringComCommitsAFrenteDaMaster)
