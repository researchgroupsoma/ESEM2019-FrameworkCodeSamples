...
dir_aux="/home/gabriel/Documentos/ic/Samples/analiseDoEstadoAtualDosSamples/spring/repositorios"
dir="/home/gabriel/Documentos/ic/Samples/analiseDoEstadoAtualDosSamples/spring/repositorios"
...
while read -r linha || [[ -n "$linha" ]] 
	do
		...
		# git log --name-status --stat --pretty=format:"$dir/$linha;%h;%s" HEAD^..HEAD > $dir/$linha.txt
		git log --pretty=%an,%ae > $dir/$linha.txt
		# echo "\n" > $dir/$pasta.txt
